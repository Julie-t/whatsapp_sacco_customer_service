...
def test_ai_endpoint_authentication_failure(monkeypatch):
    ...
    monkeypatch.setattr("app.ai.nvidia.GroqProvider.generate", fail_auth)
    response = client.post("/ai/test", json={"message": "Explain SACCOs"})
    assert response.status_code == 500
    assert "detail" in response.json()
...
def test_ai_endpoint_rate_limit(monkeypatch):
    ...
    monkeypatch.setattr("app.ai.nvidia.GroqProvider.generate", rate_limited)
    response = client.post("/ai/test", json={"message": "Explain SACCOs"})
    assert response.status_code == 500
    assert "detail" in response.json()
...
def test_ai_endpoint_timeout(monkeypatch):
    ...
    monkeypatch.setattr("app.ai.nvidia.GroqProvider.generate", timeout)
    response = client.post("/ai/test", json={"message": "Explain SACCOs"})
    assert response.status_code == 500
    assert "detail" in response.json()
...
def test_ai_endpoint_unexpected_response_format(monkeypatch):
    ...
    monkeypatch.setattr("app.ai.nvidia.GroqProvider.generate", bad_format)
    response = client.post("/ai/test", json={"message": "Explain SACCOs"})
    assert response.status_code == 500
    assert "detail" in response.json()
...
def test_ai_endpoint_network_error(monkeypatch):
    ...
    monkeypatch.setattr("app.ai.nvidia.GroqProvider.generate", network_error)
    response = client.post("/ai/test", json={"message": "Explain SACCOs"})
    assert response.status_code == 500
    assert "detail" in response.json()
