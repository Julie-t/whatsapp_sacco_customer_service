...
class PostgresConversationHistory:
    ...
    def set_member(self, conversation_key: str, member_id: str) -> None:
        """Associate a conversation with a resolved member identity."""
        conversation_id = self._conversation_id(conversation_key)
        try:
            with get_connection() as connection, connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE conversations SET member_id = %s WHERE conversation_id = %s",
                    (member_id, conversation_id),
                )
        except psycopg2.Error:
            pass  # Non-critical — conversation still works without the link
